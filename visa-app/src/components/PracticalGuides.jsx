...
export function EmergencyNumbers({ lang }) {
  const numbers = [
    ...
    { emoji: '', label: { ko: '중국대사관', zh: '中国大使馆', en: 'Chinese Embassy' }, num: '02-738-1038', desc: { ko: '영사 업무', zh: '领事服务', en: 'Consular' } },
      ...
      <div className="bg-white rounded-[14px] p-4" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center py-2">
            ...
            <span className="text-[14px] font-bold text-[#16A34A]">{L(lang, { ko: '어댑터 필요', zh: '需要转换器', en: 'Adapter needed' })}</span>
          </div>
        </div>
      </div>
    ...
    <div className="px-4 pt-4 pb-24">
      ...
      <div className="space-y-3">
        ...
        <div className="bg-[#EEF2FF] rounded-[14px] p-4">
          <p className="text-[13px] font-bold text-[#4F46E5] mb-1">{L(lang, { ko: '공항 환급 절차', zh: '机场退税流程', en: 'Airport Refund Process' })}</p>
    ...
    <div className="px-4 pt-4 pb-24">
      ...
      {selected.length > 0 && (
        <div className="bg-white rounded-[14px] p-5 text-center" style={{ boxShadow: '0 2px 12px rgba(0,0,0,0.1)', border: '2px solid #DC2626' }}>
          <p className="text-[16px] font-bold text-[#DC2626] mb-3">식품 알레르기 / 食物过敏</p>
          <p className="text-[14px] text-[#111827] mb-2">{L(lang, { ko: '아래 음식을 먹을 수 없습니다:', zh: '我不能吃以下食物：', en: 'I cannot eat the following:' })}
          </p>
          <div className="space-y-1 mb-3">{selected.map(ko => {
              const item = allergies.find(a => a.ko === ko)
              return (
                <p key={ko} className="text-[16px] font-bold text-[#DC2626]">{item.ko} / {item.zh} / {item.en}
                </p>
              )
            })}
          </div>
...
export function HallyuExperiences({ lang }) {
  ...
  return (
    <div className="px-4 pt-4 pb-24">
      ...
      <div className="space-y-3">{filtered.map(exp => (
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <a href={`https://map.kakao.com/link/to/${encodeURIComponent(L(lang, c.title))},${c.start.lat},${c.start.lng}`}
                  ...
                  className="mt-2 inline-block text-[11px] bg-[#FEE500] text-[#111827] font-bold px-3 py-1.5 rounded-full active:scale-95 transition-transform">{L(lang, { ko: '출발점 길찾기', zh: '导航到起点', en: 'Navigate to Start' })}
                </a>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="px-4 pt-4 pb-24">
                ...
                <div className="px-4 pb-4 pt-0">
                  <div className="bg-[#F9FAFB] rounded-[10px] p-3 space-y-2">
                    ...
                    <a href={`https://map.kakao.com/link/search/${encodeURIComponent(L(lang, item.name))}`}
                      ...
                      className="inline-block text-[10px] bg-[#FEE500] text-[#111827] font-bold px-2.5 py-1 rounded-full mt-1">{L(lang, { ko: '근처 맛집 찾기', zh: '搜索附近餐厅', en: 'Find nearby restaurants' })}
                    </a>
                  </div>
                </div>
              )}
      ...
      <a href="https://www.visitseoul.net/food" target="_blank" rel="noreferrer"
        className="block w-full mt-4 py-3 rounded-[12px] bg-[#111827] text-white text-[13px] font-bold text-center active:scale-95 transition-transform">{L(lang, { ko: 'Visit Seoul 맛집 더보기', zh: '查看更多Visit Seoul美食', en: 'More on Visit Seoul Food' })}
      </a>
    </div>
  )
}
...
export function TodayEvents({ lang }) {
  ...
  const categories = [
    ...
    { id: 'exhibition', label: { ko: '전시', zh: '展览', en: 'Exhibition' }, emoji: '' },
    { id: 'performance', label: { ko: '공연', zh: '演出', en: 'Performance' }, emoji: '' },
    { id: 'market', label: { ko: '마켓', zh: '市集', en: 'Market' }, emoji: '' },
  ]
  ...
  return (
    <div className="px-4 pt-4 pb-24">
      ...
      {loading ? (
        ...
        <div className="space-y-3">{filtered.map(ev => (
            <div key={ev.id} className="bg-white rounded-[14px] overflow-hidden" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              ...
              <div className="p-4">
                <div className="flex items-start gap-2">
                  <span className="text-[10px] bg-[#F3F4F6] text-[#555] px-2 py-0.5 rounded-full shrink-0">{ev.category === 'festival' ? '' : ev.category === 'exhibition' ? '' : ev.category === 'performance' ? '' : ev.category === 'market' ? '' : ''}</span>
                ...
                <div className="flex items-center gap-3 mt-2 text-[10px] text-[#9CA3AF]">{ev.date && <span>{ev.date}{ev.endDate && ev.endDate !== ev.date ? ` ~ ${ev.endDate}` : ''}</span>}
...
export function HallyuCourseMap({ lang }) {
  ...
  return (
    <div className="px-4 pt-4 pb-24">
      ...
      {current && (
        <div className="bg-white rounded-[14px] p-4 mb-4" style={{ boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
          <p className="text-[15px] font-bold text-[#111827] mb-3">{L(lang, current.title)}</p>

          {/* 코스 타임라인 */}
          <div className="relative pl-6">{current.spots.map((spot, i) => (
              <div key={i} className="relative pb-4">{/* 세로선 */}
                {i < current.spots.length - 1 && (
                  <div className="absolute left-[-16px] top-[20px] w-[2px] h-full bg-[#E5E7EB]" />
                )}
                {/* 번호 원 */}
                <div className="absolute left-[-22px] top-[2px] w-[14px] h-[14px] rounded-full bg-[#111827] text-white text-[8px] font-bold flex items-center justify-center">{i + 1}
                </div>
                {/* 장소 카드 */}
                <div className="bg-[#F9FAFB] rounded-[10px] px-3 py-2.5">
                  ...
                  <div className="flex gap-2 mt-1.5">
                    <a href={`https://map.kakao.com/link/to/${encodeURIComponent(L(lang, spot.name))},${spot.lat},${spot.lng}`}
                      target="_blank" rel="noreferrer"
                      className="text-[10px] bg-[#FEE500] text-[#111827] font-bold px-2 py-1 rounded-full active:scale-95 transition-transform">{L(lang, { ko: '길찾기', zh: '导航', en: 'Navigate' })}
                    </a>
                    <a href={`https://map.kakao.com/link/map/${encodeURIComponent(L(lang, spot.name))},${spot.lat},${spot.lng}`}
                      target="_blank" rel="noreferrer"
                      className="text-[10px] bg-[#EEF2FF] text-[#4F46E5] font-medium px-2 py-1 rounded-full active:scale-95 transition-transform">{L(lang, { ko: '지도', zh: '地图', en: 'Map' })}
                    </a>
                  </div>
                </div>
              </div>
          ...
          <a href={`https://map.kakao.com/link/to/${encodeURIComponent(L(lang, current.spots[current.spots.length - 1].name))},${current.spots[current.spots.length - 1].lat},${current.spots[current.spots.length - 1].lng}`}
            target="_blank" rel="noreferrer"
            className="block w-full mt-2 py-2.5 rounded-[10px] bg-[#111827] text-white text-[12px] font-bold text-center active:scale-95 transition-transform">{L(lang, { ko: '전체 코스 출발하기', zh: '开始全程路线', en: 'Start Full Course' })}
          </a>
        </div>
      )}
      ...
      {current && (
        <div className="flex gap-2">
          ...
          <a href={`https://map.kakao.com/link/search/${encodeURIComponent(L(lang, current.spots[0].name) + ' 카페')}`}
            target="_blank" rel="noreferrer"
            className="flex-1 py-2.5 rounded-[10px] bg-[#F0FDF4] text-[#16A34A] text-[12px] font-bold text-center active:scale-95 transition-transform">{L(lang, { ko: '근처 카페', zh: '附近咖啡', en: 'Nearby Cafe' })}
          </a>
        </div>
      )}
    </div>
        ...
        <div className="space-y-2">{spots.map(s => (
            <div key={s.id} className="bg-white rounded-[12px] px-4 py-3 flex items-center gap-3" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              ...
              <a href={`https://map.kakao.com/link/map/${encodeURIComponent(s.name)},${s.lat},${s.lng}`}
                target="_blank" rel="noreferrer"
                className="text-[10px] text-[#4F46E5] font-medium px-2 py-1 bg-[#EEF2FF] rounded-full shrink-0">{L(lang, { ko: '지도', zh: '地图', en: 'Map' })}
              </a>
            </div>
          ))}
        </div>
...
export function WeatherRecommend({ lang }) {
  ...
  return (
    <div className="px-4 pt-4 pb-24">
      ...
      <div className="space-y-2 mb-4">{rec.places.map((p, i) => (
...
const MEDICAL_ITEMS = [
  ...
  { id: 'mt-3', emoji: '', name: { ko: '안과 (라식/라섹)', zh: '眼科(LASIK/LASEK)', en: 'Ophthalmology (LASIK)' },
  ...
  return (
    <div className="px-4 pt-4 pb-24">
            ...
            <div key={item.id} className="bg-white rounded-[14px] overflow-hidden" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              ...
              {isOpen && (
                <div className="px-4 pb-4">
                  ...
                  <div className="space-y-2">{item.hospitals.map(h => (
      ...
      <a href="https://www.visitseoul.net/medical" target="_blank" rel="noreferrer"
        className="block w-full mt-4 py-3 rounded-[12px] bg-[#111827] text-white text-[13px] font-bold text-center">{L(lang, { ko: 'Visit Seoul 의료관광 더보기', zh: '查看更多Visit Seoul医疗旅游', en: 'More Medical Tourism on Visit Seoul' })}
      </a>
    </div>
  )
}
  ...
  return (
    <div className="px-4 pt-4 pb-24">
      ...
      <div className="space-y-3">{SEOUL_STAY_DATA.map(item => {
          ...
          return (
            <div key={item.id} className="bg-white rounded-[14px] overflow-hidden" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              ...
              {isOpen && (
                <div className="px-4 pb-4 space-y-2">{item.areas.map(a => (
      ...
      <a href="https://www.visitseoul.net/accommodation" target="_blank" rel="noreferrer"
        className="block w-full mt-4 py-3 rounded-[12px] bg-[#111827] text-white text-[13px] font-bold text-center">{L(lang, { ko: 'Visit Seoul 숙소 더보기', zh: '查看更多Visit Seoul住宿', en: 'More on Visit Seoul Accommodation' })}
      </a>
    </div>
  )
}
...
const CULTURE_LOUNGE_DATA = [
  ...
  { id: 'cl-4', emoji: '', name: { ko: 'DDP (동대문디자인플라자)', zh: 'DDP(东大门设计广场)', en: 'DDP (Dongdaemun Design Plaza)' },
  ...
  return (
    <div className="px-4 pt-4 pb-24">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex gap-2 mt-2">
                  <a href={`https://map.kakao.com/link/to/${encodeURIComponent(L(lang, item.name))},${item.lat},${item.lng}`}
                    target="_blank" rel="noreferrer"
                    className="text-[10px] bg-[#FEE500] text-[#111827] font-bold px-2.5 py-1 rounded-full active:scale-95 transition-transform">{L(lang, { ko: '길찾기', zh: '导航', en: 'Navigate' })}
                  </a>
                  <a href={`https://map.kakao.com/link/map/${encodeURIComponent(L(lang, item.name))},${item.lat},${item.lng}`}
                    target="_blank" rel="noreferrer"
                    className="text-[10px] bg-[#EEF2FF] text-[#4F46E5] font-medium px-2.5 py-1 rounded-full active:scale-95 transition-transform">{L(lang, { ko: '지도', zh: '地图', en: 'Map' })}
                  </a>
                </div>
              </div>
            </div>
      ...
      <a href="https://www.visitseoul.net/culture" target="_blank" rel="noreferrer"
        className="block w-full mt-4 py-3 rounded-[12px] bg-[#111827] text-white text-[13px] font-bold text-center">{L(lang, { ko: 'Visit Seoul 문화관광 더보기', zh: '查看更多Visit Seoul文化旅游', en: 'More Culture on Visit Seoul' })}
      </a>
    </div>
  )
}
