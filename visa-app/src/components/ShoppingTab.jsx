...
import { WidgetContent, LucideIcon, SECTION_SHOPPING, getEnabledWidgetsForSection } from './HomeTab'
...
// Add taxrefund to shopping list
const SHOPPING_IDS = [...SECTION_SHOPPING, 'taxrefund']

export default function ShoppingTab({ lang, setTab }) {
  const [activeWidget, setActiveWidget] = useState(null)

  // Get all shopping widgets from widget categories
  const allWidgets = []
  widgetCategories.forEach(cat => {
    cat.widgets.forEach(w => {
      if (SHOPPING_IDS.includes(w.id)) allWidgets.push(w)
    })
  })
