...
function Logo({ size = 'md', color = 'dark' }) {
  const scales = { sm: 0.7, md: 0.9, lg: 1.15 }
  const sc = scales[size] || scales.md
  const textColor = color === 'white' ? '#ffffff' : '#111827'
  return (
    <svg width={160 * sc} height={28 * sc} viewBox="0 0 160 28" fill="none" style={{ overflow: 'visible' }}>
      <text x="80" y="19" textAnchor="middle" fontFamily="'Caveat', cursive" fontWeight="700" fontSize="24" fill={textColor}>
        NEAR
      </text>
    </svg>
  )
...
export default function OnboardingFlow({ onComplete, lang, setLang }) {
  ...
  useEffect(() => {
    if (step === 'splash') {
      const timer = setTimeout(() => setStep('login'), 1000)
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 relative">
        ...
        {step !== 'splash' && (
          <button onClick={() => setLang(nextLang(lang))}
            className="absolute top-6 right-6 text-white text-sm px-3 py-1.5 rounded-full border border-white/40 hover:border-white transition-all z-10">
            {langLabel(lang)}
          </button>
        )}
          ...
          <div className="w-full max-w-sm animate-fade-up">
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 bg-[#FEE500] text-[#3C1E1E] rounded-xl p-4 font-medium hover:bg-[#FDD835] transition-all btn-press ">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 bg-[#03C75A] text-white rounded-xl p-4 font-medium hover:opacity-90 transition-all btn-press ">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 bg-[#07C160] text-white rounded-xl p-4 font-medium hover:opacity-90 transition-all btn-press ">
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-3 bg-[#1677FF] text-white rounded-xl p-4 font-medium hover:opacity-90 transition-all btn-press ">
